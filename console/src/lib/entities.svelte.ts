import type { Mint, Persona } from './types';
...
// static; addresses start empty and are filled in place (same reactive
// object, mutated fields) from the server's real devnet state on the first
// successful fetch — see store/demo-store.svelte.ts. Every component reading
// these therefore re-renders with real addresses after that fetch.
export const MINT: Mint = $state({
	address: '',
	name: 'Token-X',
	symbol: 'TOKEN-X',
	decimals: 2,
...
export const SENDER: Persona = $state({
	id: 'sender',
...
export const RECEIVER: Persona = $state({
	id: 'receiver',
