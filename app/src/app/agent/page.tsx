...
// The route is kept so old links don't 404; it just sends visitors back to the dashboard.
// To restore: delete the stub below and uncomment the original page that follows.
...
// ---------------------------------------------------------------------------
// Original Agent Payments page (commented out)
// ---------------------------------------------------------------------------
// "use client";
//
...
// import { AGENT_ACTING_ACCOUNT_ID, useDemoStore } from "@/store/demo-store";
...
//   // Pinned, not whatever the sidebar last selected — see
//   // AGENT_ACTING_ACCOUNT_ID.
//   const payerPersona = findPersona(AGENT_ACTING_ACCOUNT_ID);
//
//   const [intent, setIntent] = useState("");
//   // One per page: a proposal card carries the intent, parsed fields, three
//   // data-disclosure panels, the policy checks and its actions — a second one
//   // pushes the first entirely off screen anyway.
//   const proposalPage = usePagination(agentProposals, 1);
//
//   return (
//     <>
//       <PageHeader
...
//               <Avatar initials={payerPersona.initials} seed={payerPersona.id} size="sm" />
//               <div>
//                 <p className="text-ink-400">{c.agent.actingForLabel}</p>
//                 <p className="font-semibold text-ink-900">{payerPersona.name}</p>
//               </div>
//             </div>
//           )
//         }
...
//
//         <Card>
//           <CardHeader title={c.agent.newInstructionTitle} subtitle={c.agent.newInstructionSubtitle(payerPersona?.name ?? "")} />
...
//           ))}
//           {proposalPage.showControls && (
//             <div className="rounded-xl border border-border-subtle">
//               <PaginationControls
//                 page={proposalPage.page}
//                 pageCount={proposalPage.pageCount}
//                 hasPrev={proposalPage.hasPrev}
//                 hasNext={proposalPage.hasNext}
//                 onPrev={proposalPage.prev}
//                 onNext={proposalPage.next}
//               />
//             </div>
//           )}
//         </div>
...
//   const abandonAgentProposal = useDemoStore((s) => s.abandonAgentProposal);
...
//                 </option>
//                 {/* Never offer the account being paid from — the drawer
//                     filters the same way; without it you can pick yourself. */}
//                 {PERSONAS.filter((p) => p.id !== AGENT_ACTING_ACCOUNT_ID).map((p) => (
...
//         </div>
//
//         {(proposal.status === "needs_clarification" || proposal.status === "policy_review") && (
//           // One row, so giving up reads as the way out rather than the first
//           // thing to do: the action that moves the proposal forward leads on
//           // the left, and abandoning is pushed to the far right. An
//           // instruction the agent can't resolve still needs an exit, or it
//           // sits here forever with no way out but answering it.
//           <div className="flex flex-wrap items-center gap-3">
//             {proposal.status === "policy_review" && !proposal.parsed.ambiguous && (
//               <Button onClick={() => runPolicyChecks(proposal.id)}>
...
//               variant="secondary"
//               className="ml-auto"
//               onClick={() => abandonAgentProposal(proposal.id)}
//             >
//               <CircleSlash size={14} /> {c.agent.abandonInstruction}
//             </Button>
//           </div>
//         )}
//
//         {proposal.status === "abandoned" && (
//           <div className="flex items-center gap-3 rounded-xl border border-border-subtle bg-canvas/60 p-4 text-sm text-ink-500">
//             <p>{c.agent.abandonedNote}</p>
//             {proposal.decisionRecordId && (
//               <Button
//                 size="sm"
//                 variant="secondary"
...
//                     : check.key === "availableBalance" && check.status === "requires_approval"
//                     ? c.policy.availableBalance.unknown
//                     : check.status === "pass"
//                     ? c.policy[check.key].pass
//                     : check.key === "recipientAllowlist" && check.failReason === "selfPayment"
//                     ? c.policy.recipientAllowlist.failSelf
...
//         )}
//
//         {proposal.status === "rejected" && proposal.decisionRecordId && (
//           // Both refusals are recorded and disclosed like any other agent
//           // decision, so offer the same way in — but say which one it was:
//           // rules firing and a person saying no are different events.
//           <div className="flex items-center gap-3 rounded-xl border border-danger-100 bg-danger-50 p-4 text-sm text-danger-600">
//             <p className="font-medium">
//               {proposal.policyChecks.some((ch) => ch.status === "fail")
//                 ? c.agent.blockedByPolicyTitle
//                 : c.agent.declinedByOperatorTitle}
//             </p>
//             <Button
//               size="sm"
//               variant="secondary"
...
//     abandoned: c.agent.status.abandoned,
//   }[status];
//
//   return (
