...
//! range), pre-verifies each into a context state account, then references
//! those accounts in `spl-token-2022`'s transfer instruction via
//! `ProofLocation::ContextStateAccount`. The oversized range proof is staged
//! through an spl-record account since it doesn't fit in a single legacy
//! (1232-byte) transaction alongside the verify instruction.
//!
...
pub async fn transfer_confidential_with_progress(
    ...
    phase(
        ...
        "Allocating + verifying proof context state accounts",
    );
    ...
    steps.push(LabeledSignature::new("verify_validity_proof", &sig));
    signatures.push(sig);

    // 3. Range proof — staged via an spl-record account (too big for a single
    // verify instruction alongside its own context-account creation).
    ...
    {
        (
            Some(hex::encode(bytemuck::bytes_of(&auditor_lo_legacy))),
            Some(hex::encode(bytemuck::bytes_of(&auditor_hi_legacy))),
        )
    } else {
        (None, None)
    };
    ...
    steps.push(LabeledSignature::new("submit_transfer", &sig));
    signatures.push(sig);
